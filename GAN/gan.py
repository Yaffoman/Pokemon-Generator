...
def build_networks():
    ...
    dopt = Adam(lr=0.0001, beta_1=Args.adam_beta)
    opt = Adam(lr=0.00008, beta_1=Args.adam_beta)
...
def train_autoenc(dataf):
    ...
    while epoch < 200:
        ...
        dump_batch(fakes, 4, "fakes_{}.png".format(epoch))
        dump_batch(reals, 4, "reals_{}.png".format(epoch))
...
def train_gan(dataf):
    ...
    load_weights(disc, "snapshots/9.disc.hdf5")
    ...
    with h5py.File(dataf, 'r') as f:
        faces = f.get('faces')
        run_batches(gen, disc, gan, faces, logger, range(15000, 30000))
    logger.on_train_end()
...
def run_batches(gen, disc, gan, faces, logger, itr_generator):
    ...
    for batch in itr_generator:
        ...
        # print(fakes.shape, lbl_fake.shape)
...
def generate(genw, cnt):
    ...
    generated = gen.predict(binary_noise(cnt))
    # Unoffset, in batch.
